  ...
  useEffect(() => {
    if (messages.length === 0 && !isLoading && topicProgress.length > 0) {
      ...
      if (topicsToImprove.length > 0) {
        ...
        recommendationMessage += `📖 [Изучить теорию](/textbook2) - основные понятия и признаки\n`;
        recommendationMessage += `📺 [Посмотреть видеоурок](/videos) - подробное объяснение с примерами\n`;
        recommendationMessage += `📝 [Решать задачи](/practice) - практические упражнения\n`;
        recommendationMessage += `🏦 [Банк заданий ФИПИ](/fipi-bank) - официальные задачи по этой теме\n\n`;
        recommendationMessage += `Начни с теории, затем посмотри видео и переходи к практике! 🚀`;
      }
