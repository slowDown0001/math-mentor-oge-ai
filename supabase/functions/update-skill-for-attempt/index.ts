...
Deno.serve(async (req)=>{
  ...
  try {
    const { user_id, skills, finished_or_not, is_correct, difficulty, scaling_type, course_id } = await req.json();
    ...
    if (typeof user_id !== 'string' || !Array.isArray(skills) || typeof finished_or_not !== 'boolean' || typeof is_correct !== 'boolean' || typeof difficulty !== 'number' || typeof scaling_type !== 'string') {
      return new Response(JSON.stringify({
